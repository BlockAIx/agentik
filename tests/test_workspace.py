...
class TestCommitAndMergeSelective:
    def test_selective_adds_only_listed_files(
        ...
        def fake_git_run_checked(cmd: str, proj: Path, desc: str) -> bool:
            calls.append(cmd)
            return True

        monkeypatch.setattr("runner.workspace.git_run", fake_git_run)
        monkeypatch.setattr("runner.workspace._git_run_checked", fake_git_run_checked)
...
class TestInstallDependenciesForceFlag:
    """Verify that install_project_dependencies calls pnpm without --force."""
