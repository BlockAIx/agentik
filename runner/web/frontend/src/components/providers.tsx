...
/** Shape returned by the new /api/providers/login endpoint. */
...
/** Provider management panel — auth status, available models, login/logout. */
export function Providers(): React.JSX.Element {
  ...
  const handleLogin = async () => {
    // Toggle: close the guide if already open.
    if (loginGuide) {
      setLoginGuide(null)
    ...
    setError(null)
    try {
      const result = await api.providerLogin()
  ...
  const handleRefreshModels = async () => {
    setRefreshing(true)
    try {
      const result = await api.refreshAvailableModels()
  ...
  const hasGithubCopilot = providers.some(
    (p) => p.name.toLowerCase().includes("github copilot") || p.name.toLowerCase().includes("copilot"),
  )
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            {!hasGithubCopilot ? (
              ...
              >
                ...
                {loggingIn ? "Loading..." : loginGuide ? "Hide instructions" : "Connect GitHub Copilot"}
              </Button>
            ) : (
              <Button
                onClick={handleLogin}
              ...
              >
                <LogIn className="h-3.5 w-3.5 mr-1.5" />
                {loggingIn ? "Loading..." : loginGuide ? "Hide instructions" : "Add Provider"}
              </Button>
            )}
            {providers.length > 0 && (
              <Button
          ...
          {/* Connection guide — replaces the old subprocess output box */}
          {loginGuide && (
            <ConnectGuide
              guide={loginGuide}
          ...
          <p className="text-xs text-muted-foreground">
            ...
            your terminal and follow the on-screen steps.{" "}
            API providers (Anthropic, OpenAI) use environment variables (
            <code className="bg-muted px-1 rounded text-xs">
              ANTHROPIC_API_KEY
            </code>
            ,{" "}
            <code className="bg-muted px-1 rounded text-xs">OPENAI_API_KEY</code>
            ).
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            These are the models available through your connected providers.
            Use the model ID (e.g.{" "}
            <code className="bg-muted px-1 rounded text-xs">github-copilot/claude-sonnet-4</code>)
...
function ConnectGuide({
  ...
  onRefresh,
}: ConnectGuideProps): React.JSX.Element {
  ...
  return (
    <div className="rounded-md border border-blue-500/20 bg-blue-500/5 space-y-4 p-4">
      ...
      {/* Refresh button after completing auth */}
...
function ModelRow({ model }: { model: AvailableModel }): React.JSX.Element {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(model.full_id).then(() => {
      setCopied(true)
